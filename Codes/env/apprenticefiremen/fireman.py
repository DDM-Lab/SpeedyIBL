...
class FIREMEN(object):
    ...
    def reset(self):
        ...
        self.FIRE_Y = self.c.FIRE_Y()
        self.FIRE_X = self.c.FIRE_X()
        # Obstacles, agents and goods are initialised:
        self.setObstacles()
        self.setTools()
        self.s = [copy.deepcopy(self.s_t),copy.deepcopy(self.s_t)]
        self.initAgents()
        self.delivered = False
        self.AGENTS = np.copy(self.c.AGENTS)
        self.choices = [0,0]
    ...
    def setTools(self):
        ...
        for tool, x, y in self.c.PICKUP_LOCATIONS:
            self.s_t[y][x] = self.c.TOOLS[tool]
        self.s_t[self.FIRE_Y][self.FIRE_X] = self.c.FIRE
    ...
    def goodsPickup(self):
        ...
        for j in range(self.c.NUMBER_OF_AGENTS):
            if (self.holding_goods[j] == False):
                for tool, x, y in self.c.PICKUP_LOCATIONS:
                    if (self.agents_x[j] == x and  
                    self.agents_y[j] == y):
                        self.choices[j] = self.c.TOOLS[tool]
    ...
    def goodsDelivered(self):
        ...
        '''
        for j in range(self.c.NUMBER_OF_AGENTS):
            if (self.agents_x[j] == self.FIRE_X and self.agents_y[j] == self.FIRE_Y and 
                self.holding_goods[j] == True):
                self.arriveds[j] = True

        if self.arriveds[0] and self.arriveds[1]:        
            for (a, b, r) in self.c.REWARDS:
                if (self.choices[0] == a and self.choices[1] == b):
                    self.delivered = True
                    return [r(),r()]
    ...
    def moveAgents(self, actions):
       ...
       for i in range(self.c.NUMBER_OF_AGENTS):
           if self.arriveds[i] == False:
                ...
                if  self.holding_goods[i]:
                    if self.noCollision_1(targetx, targety, i) or (targetx == self.FIRE_X and targety == self.FIRE_Y):
                        self.moveAgent(i, targetx, targety)
                elif self.noCollision(targetx, targety, i):
                    self.moveAgent(i, targetx, targety)
    ...
    def moveAgent(self, id, targetx, targety):
        ...
        '''
        self.s_t[self.agents_y[id]][self.agents_x[id]] -= self.AGENTS[id]
        self.s[id][self.agents_y[id]][self.agents_x[id]] -= self.AGENTS[id]
    ...
    def noCollision(self, x, y, id):
        ...
        if x < 0 or x >= self.c.GW or\
           ...
           self.s[1-id][y][x] == self.AGENTS[1-id] or\
           self.s[id][y][x] == self.c.OBSTACLE or\
           self.s[id][y][x] == self.c.FIRE:
            return False
        else:
            return True
    def noCollision_1(self, x, y, id):
        '''
        Checks if x, y coordinate is currently empty 
        :param x: Int, x coordinate
        :param y: Int, y coordinate
