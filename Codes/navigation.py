...
for runid in range(FLAGS.start_t,FLAGS.trials):
    ...
    for i in range(FLAGS.agents): 
        ...
        if FLAGS.type == "libl":
            agents.append(Agent(agent_config,default_utility=2.5,lendeque=5000)) # Init agent instances
        if FLAGS.type == "ibl" or FLAGS.type == "latestibl":
            agents.append(AgentPyIBL(agent_config,default_utility=2.5))
    ...
    for i in range(FLAGS.episodes):
        ...
        for j in range(FLAGS.steps):
            ...
            for agent, o, r, action, x, y in zip(agents, observations, rewards, actions, agents_x, agents_y):
                ...
                # if y + dy > env.env.c.GH-1 or x + dx > env.env.c.GW-1:
                #     r = -0.05
                # if r == 0:
                #     r = -0.01
